<script>
  export let head = 'Welcome.';
  export let subhead = "Bienvenido.";
  let animate = true;
...
<h1 class="welcome">
  <div class="highlight" style={`--animation-length: 2s;`}>
    ...
    <div class={`highlight__bg-wrap`} class:enter={animate}>
      <div class="highlight__bg" class:enter={animate}>{head}<br><span class="subhead">{subhead}</span></div>
    </div>

    <div class="highlight__first-word" class:enter={animate}>
      {head}<br><span class="subhead">{subhead}</span>
    </div>
